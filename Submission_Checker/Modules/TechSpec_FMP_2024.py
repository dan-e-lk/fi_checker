#-------------------------------------------------------------------------------
# Name:         TechSpec_FMP_2024.py
# Purpose:      This module checkes every validation statements in FMP Tech Spec 2024
#
# Author:       Policy Division, Ministry of Natural Resources
#
#
# Created:      Aug 2024 - DAN.  starting point is TechSpec_FMP_NEW.py
# Updates:      
#-------------------------------------------------------------------------------
...
# Lyr acronym            name                           mandatory fields                                            Data type   Tech Spec       Tech Spec URL
    ...
    "BMI":  ["Base Model Inventory",                    ['POLYID', 'POLYTYPE', 'OWNER', 'YRSOURCE',
                                                       ...
                                                       'AGE', 'AC5', 'AC10','AC20', 'HT', 'SI', 'BA', 'CCLO', 'STKG', 'SC', 'MANAGED',
                                                       'SMZ', 'LGFU', 'PLANFU', 'AU', 'AVAIL', 'SILVSYS', 
                                                       'NEXTSTG', 'YIELD'],                                         'polygon',  '4.1.4',        R.findPDF('FIM_FMP_TechSpec_2020.pdf#page=11')], # Note that 'SMZ' is no longer mandatory
    ...
    "OPI":  ["Operational Planning Inventory",          ['POLYID', 'POLYTYPE', 'OWNER', 'YRSOURCE',
                                                       ...
                                                       'AGE', 'AC5', 'AC10','AC20', 'HT', 'SI', 'BA', 'CCLO', 'STKG', 'SC', 'MANAGED', 
                                                       'SMZ', 'LGFU', 'PLANFU', 'AU', 'AVAIL', 'SILVSYS', 
                                                       'NEXTSTG', 'YIELD', 'OMZ', 'SGR'],                           'polygon',  '4.1.4',        R.findPDF('FIM_FMP_TechSpec_2020.pdf#page=11')], # Note that 'SMZ' is no longer mandatory
    ...
    "PCI":  ["Planning Composite",                      ['POLYID', 'POLYTYPE', 'OWNER', 'YRSOURCE',
                                                        ...
                                                        'UHT', 'UCCLO', 'USTKG', 'USC', 'INCIDSPC', 'VERT', 
                                                        'PRI_ECO', 'SEC_ECO', 'ACCESS1', 'ACCESS2','MGMTCON1',
                                                       'MGMTCON2', 'MGMTCON3'],                                     'polygon',  '4.1.4',        R.findPDF('FIM_FMP_TechSpec_2020.pdf#page=11')],
...
#       ######### Going through each layer type. Starts with PCI BMI OPI FDP, then in alphabetical order ##########
        ...
        if lyrAcro in ["PCI","BMI","OPI"]:
            ...
            try:
                ...
                ## The following statement can be checked on other fields: "If POLYTYPE attribute does not equal FOR, then DEVSTAGE, OYRORG, OSPCOMP..."
            ...
            # OWNER *2024.09 big changes in ownership
            try:
                current_field = 'OWNER'
                arcpy.AddMessage("\tChecking %s..."%current_field) #*2023.01      
                errorList = ["Error on %s %s: The population of OWNER is mandatory and must follow the correct coding scheme."%(id_field, cursor[id_field_idx]) for row in cursor
                                if str(cursor[f.index('OWNER')]) not in ['CROWN','TIMCRN','PATENT','PARKS','FN','FEDRES']]
                cursor.reset()
                if len(errorList) > 0:
                    errorDetail[lyr].append(errorList)
                    criticalError += 1
            ...
            # SOURCE *2024.09 addition of LIDAR
            try:
                ...
                errorList = ["Error on %s %s: The population of SOURCE must follow the correct coding scheme."%(id_field, cursor[id_field_idx]) for row in cursor
                                if cursor[f.index('SOURCE')] not in ['BASECOVR','DIGITALA','DIGITALP','ESTIMATE','FOC','FORECAST','FRICNVRT','INFRARED','LIDAR','MARKING','OCULARA','OCULARG','OPC','PHOTO','PHOTOLS','PHOTOSS','PLOTFIXD','PLOTVAR','RADAR','REGENASS','SEMEXTEN','SEMINTEN','SPECTRAL','SUPINFO']]
                cursor.reset()
                if len(errorList) > 0:
                    errorDetail[lyr].append(errorList)
                    criticalError += 1
            ...
            try:
                ...
                e1List, e2List, e3List, w1List, w4List = [],[],[],[],[]
                for row in cursor:
                    if cursor[f.index(current_field)] not in vnull:
                        check = R.spcVal(cursor[f.index(current_field)],current_field,2024)
                        ...
                        elif check[0] == "Warning4":
                            w4List.append("Warning on %s %s: %s"%(id_field, cursor[id_field_idx],check[1]))
                cursor.reset()
                    # summarizing errors
                if len(e1List + e2List + e3List) > 0:
                    criticalError += 1
                for errorlist in [e1List, e2List, e3List]:
                    if len(errorlist) > 0:
                        errorDetail[lyr].append(errorlist)
                ...
                if len(w1List) > 0:
                    minorError += 1
                for warninglist in [w1List, w4List]:
                    if len(warninglist) > 0:
                        errorDetail[lyr].append(warninglist)
                        recordValCom[lyr].append("Warning on %s record(s):%s."%(len(warninglist),warninglist[0][warninglist[0].find(':')+1:]))
            except ValueError:
                recordValCom[lyr].append("***Unable to run full validation on %s field due to value error - most likely due to missing mandatory field(s)"%current_field)
                arcpy.AddWarning("***Unable to run full validation on %s field due to the following error:\n"%current_field + str(sys.exc_info()[1]))
                criticalError += 1  
...
# continue editing from here (also need to test OSPCOMP validation and apply it to USPCOMP and SPCOMP)
            ...
            try:
                ...
                if lyrAcro in ["BMI", "OPI"]:
                    ...
                    errorList = ["Warning on %s %s: AVAIL should be U when OWNER = 1."%(id_field, cursor[id_field_idx]) for row in cursor
                                    if cursor[f.index('POLYTYPE')] == 'FOR'
                    ...
                    if len(errorList) > 0:
                        errorDetail[lyr].append(errorList)
                        minorError += 1
                        recordValCom[lyr].append("Warning on %s record(s): AVAIL should be U when OWNER = 1."%len(errorList))
